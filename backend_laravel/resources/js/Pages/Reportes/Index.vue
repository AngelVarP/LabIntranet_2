...
const data = ref({ solicitudes:{}, prestamos:{}, totales:{ solicitudes:0, prestamos:0 } })
const estado = ref('')
...
async function load(){
  ...
  try{
    const { data:res } = await axios.get('/api/reportes/resumen')
    data.value = res
    ...
    <div class="p-6 space-y-6">
      ...
      <div class="grid md:grid-cols-3 gap-4">
        <div class="rounded-2xl bg-white p-4 shadow-sm border">
          ...
          <div class="text-3xl font-semibold">{{ data.totales?.solicitudes ?? 0 }}</div>
          <div class="mt-2 text-xs text-slate-500">
            <span v-for="(n,est) in data.solicitudes" :key="est" class="mr-3">{{ est }}: {{ n }}</span>
          </div>
        </div>
        <div class="rounded-2xl bg-white p-4 shadow-sm border">
          <div class="text-slate-500 text-sm">Total Préstamos</div>
          <div class="text-3xl font-semibold">{{ data.totales?.prestamos ?? 0 }}</div>
          <div class="mt-2 text-xs text-slate-500">
            <span v-for="(n,est) in data.prestamos" :key="est" class="mr-3">{{ est }}: {{ n }}</span>
          </div>
        </div>
        <div class="rounded-2xl bg-white p-4 shadow-sm border">
          ...
          <button class="mt-3 rounded border px-3 py-2 hover:bg-slate-50" @click="exportCSV">Exportar CSV</button>
        </div>
      </div>
